...
async def test_retry_after_http_date_parsed(monkeypatch):
    ...
    # Ensure we observed a sleep of roughly 2 seconds (or at least >=1.9)
    assert any(d >= 1.9 for d in sleep_calls)
    assert res.status == "OK" or getattr(res, "status", None) == "OK" or res is not None
